...
# The Consolidated Monthly P&L Summary table is designed and finalized by
# hand each month directly inside the board deck (think-cell table, native
# PowerPoint numbers) -- that deck IS the single source of truth the board
# actually sees, so we read the table straight from there instead of
# recomputing it from the Excel model (which can drift from the deck's own
# manual adjustments). "2. Monthly" sits several levels above ROOT, as a
# sibling of "0. VF" -- see the folder tree comment above BOARD_XLSX_CANDIDATES.
MONTHLY_DIR_CANDIDATES = [
    os.path.normpath(os.path.join(ROOT, '..', '..', '..', '..', '..', '..', '2. Monthly')),
]

PPT_ROW_LABELS = {
    'revenues (net of interco)': 'revenue',
    'cogs + opex': 'opex',
    ...
    'd&a': 'da',
    'ebit': 'ebit',
    'net interest': 'interest_expense',
    'ebt': 'ebt',
    'taxes': 'taxes',
    'normalized net income': 'net_income',
    'net margin': 'net_margin',
}
PPT_ACCOUNTING_LABELS = {
    'accounting ebitda', 'adj. accounting ebitda', 'accounting net income',
}
# fallback P&L Managerial Notes, used only if the deck's own notes textbox
# can't be found on the slide (kept in sync with the deck as of Jul-2026)
DEFAULT_PL_NOTES_TITLE_BOLD = 'P&L Managerial Notes:'
DEFAULT_PL_NOTES_TITLE_REST = ' For comparability purposes vs. Budget:'
DEFAULT_PL_NOTES = [
    'Certain Revenue, OPEX and SG&A accounts are reclassified vs. accounting figures.',
    'For 2026, VCN’s results are normalized for the IFRS 16 treatment of hub lease rent. '
    'Specifically, we present actual rent expense within OPEX (i.e., not within Depreciation '
    'and Interest). This normalization is applied only to hubs whose contractual grace periods '
    'have ended, as opposed to the accounting treatment, which recognizes IFRS 16 impacts for '
    'hubs already signed even if they are still within the grace period.',
    'Excludes FX gain / (loss).',
]


def _clean_ppt_text(txt):
    """Normaliza texto extraido de una celda/parrafo de PPTX: colapsa vertical
    tabs (line breaks dentro de una celda), NBSP y saltos de linea a espacios
    simples, y recorta espacios repetidos."""
    if txt is None:
        return ''
    t = txt.replace(chr(160), ' ').replace(chr(11), ' ').replace(chr(10), ' ')
    return ' '.join(t.split())


def _norm_ppt_label(s):
    """Normaliza un label de fila para matchear contra PPT_ROW_LABELS: quita
    el numero de nota al pie (superindice, ej. 'COGS + OPEX2' -> 'cogs + opex'),
    colapsa espacios y pasa a minusculas."""
    s = _clean_ppt_text(s)
    s = re.sub(r'\d+$', '', s).strip()
    return s.lower()


def _parse_ppt_num(txt):
    """Convierte el texto ya formateado de una celda ('$128.8', '($90.9)',
    ' 33.0% ', 'n.m.', '') al numero (float) que representa, o None si la
    celda esta vacia / no aplica. Los porcentajes de las filas de margen se
    devuelven en puntos porcentuales (40.4, no 0.404) -- mismo esquema que ya
    usa consolidated_pl en el resto del pipeline."""
    t = _clean_ppt_text(txt)
    if t == '' or t == '-' or t.lower() in ('n.m.', 'n.a.'):
        return None
...
def find_monthly_root():
    for cand in MONTHLY_DIR_CANDIDATES:
        if os.path.isdir(cand):
            return cand
    return None


def _list_year_dirs(monthly_root):
    out = []
    try:
        for name in os.listdir(monthly_root):
            if re.match(r'^\d{4}$', name) and os.path.isdir(os.path.join(monthly_root, name)):
                out.append(name)
    except OSError:
        return []
...
def _list_month_dirs(year_dir):
    out = []
    try:
        for name in os.listdir(year_dir):
            m = re.match(r'^(\d{2})\s', name)
            if m and os.path.isdir(os.path.join(year_dir, name)):
...
def _find_review_pptx(month_dir):
    """Dentro de una carpeta de mes (ej. '07 Julio 2026'), busca el deck
    consolidado 'VEMO - <Mes> Financial Review.pptx' -- ignora las copias
    ('Copia'), los decks por-empresa ('VCN - ...') y los archivos de bloqueo
    temporales ('~$...'). Si hay version '(Audited)' y version normal,
    prefiere la normal (se publica primero, el audited puede tardar)."""
    candidates = []
    try:
    ...
    for name in names:
        low = name.lower()
        if not low.endswith('.pptx') or name.startswith('~$'):
            continue
        if not low.startswith('vemo'):
            continue
        if 'financial review' not in low:
            continue
        if 'copia' in low:
            continue
    ...
    if not candidates:
        return None
    non_audited = [c for c in candidates if 'audited' not in c.lower()]
    pool = non_audited if non_audited else candidates
    pool.sort(key=len)
    return os.path.join(month_dir, pool[0])


def extract_ppt_pl_slide(path):
    """Abre un deck 'VEMO - <Mes> Financial Review.pptx' y busca la slide
    titulada 'Consolidated Monthly - P&L Summary (Managerial View)', lee su
    tabla nativa de PowerPoint (think-cell) celda por celda, y arma:
      - pl: {field: {yoy, prev, cur, ytd, bud_cur, bud_ytd}} -- mismo esquema
        que ya usa consolidated_pl (fallback board_xlsx / summed)
      - accounting: lista de filas de referencia (Accounting EBITDA, Adj.
        Accounting EBITDA, Accounting Net Income) con sus 4 columnas Actuals
        y la nota explicativa ('->')
      - notes_title / notes: el texto real del cuadro 'P&L Managerial Notes'
        de esa misma slide (buscado por su primera linea, no por nombre de
        forma, ya que el nombre puede variar mes a mes)
    Devuelve None si la slide/tabla no se encuentra, o si la tabla existe pero
    todavia esta en blanco (deck del mes en curso, aun no llenado) -- el
    llamador entonces prueba el mes anterior."""
    ...
    for slide in prs.slides:
        for shp in slide.shapes:
            if shp.has_text_frame:
                low = shp.text_frame.text.lower()
                if 'consolidated monthly' in low and 'p&l summary' in low:
                    target_slide = slide
                    break
    ...
    for row in table.rows:
        ...
        if not cells:
            continue
        label = _clean_ppt_text(cells[0].text)
        if not label:
            continue
        norm = _norm_ppt_label(label)

        def cellnum(idx):
            return _parse_ppt_num(cells[idx].text) if idx < len(cells) else None

        if norm in PPT_ROW_LABELS:
            key = PPT_ROW_LABELS[norm]
            # the deck's P&L (MXNm) table already shows figures in millions
            # (e.g. "$96.8" = $96.8m); the rest of this pipeline (fmtMoney /
            # fmtMm client-side) expects raw currency units for every money
            # field, dividing by 1e6 itself at render time -- so scale money
            # rows back up by 1e6 here. Margin rows are percentage points
            # already (e.g. 40.4 = 40.4%) and must NOT be scaled.
            scale = 1e6 if key not in MARGIN_FIELDS else 1
            def sc(v):
                return None if v is None else v * scale
            pl[key] = {
                'yoy': sc(cellnum(1)), 'prev': sc(cellnum(2)), 'cur': sc(cellnum(3)), 'ytd': sc(cellnum(4)),
                'bud_cur': sc(cellnum(6)), 'bud_ytd': sc(cellnum(7)),
            }
        elif norm in PPT_ACCOUNTING_LABELS:
            note = _clean_ppt_text(cells[7].text) if len(cells) > 7 else ''
            def sc2(v):
                return None if v is None else v * 1e6
            accounting.append({
                'label': label, 'note': note,
                'yoy': sc2(cellnum(1)), 'prev': sc2(cellnum(2)), 'cur': sc2(cellnum(3)), 'ytd': sc2(cellnum(4)),
            })

    required = ('revenue', 'gross_profit', 'ebitda', 'net_income')
    if any(k not in pl for k in required) or pl['revenue'].get('cur') is None:
        # la tabla existe (labels ya puestos por el template del deck) pero
        # todavia no se ha llenado con los numeros del mes -- no es una
        # fuente valida, el llamador cae al mes anterior.
        return None

    notes_title_bold, notes_title_rest, notes = None, '', []
    for shp in target_slide.shapes:
        if shp.has_table or not shp.has_text_frame:
            continue
        paras = shp.text_frame.paragraphs
        if not paras:
            continue
        first_clean = _clean_ppt_text(paras[0].text)
        if not first_clean.lower().startswith('p&l managerial notes'):
            continue
        # split the title paragraph at its bold/non-bold run boundary (the
        # deck bolds just "P&L Managerial Notes:", the rest is regular
        # weight) so the dashboard reproduces the same emphasis, not an
        # all-bold or all-regular title.
        bold_parts, rest_parts, in_bold = [], [], True
        for run in paras[0].runs:
            is_bold = bool(run.font.bold)
            if is_bold and in_bold:
                bold_parts.append(run.text)
            else:
                in_bold = False
                rest_parts.append(run.text)
        notes_title_bold = _clean_ppt_text(''.join(bold_parts)) or first_clean
        notes_title_rest = _clean_ppt_text(''.join(rest_parts))
        notes = [_clean_ppt_text(p.text) for p in paras[1:] if _clean_ppt_text(p.text)]
        break

    return {
        'pl': pl, 'accounting': accounting,
        'notes_title_bold': notes_title_bold or DEFAULT_PL_NOTES_TITLE_BOLD,
        'notes_title_rest': notes_title_rest or DEFAULT_PL_NOTES_TITLE_REST,
        'notes': notes or DEFAULT_PL_NOTES,
    }


def find_latest_ppt_pl_data():
    """Recorre '2. Monthly/<anio>/<NN Mes>/' del mas reciente al mas antiguo
    (anio descendente, luego mes descendente dentro del anio) buscando el
    primer deck 'VEMO - <Mes> Financial Review.pptx' cuya tabla de P&L
    Consolidado ya este llena -- asi, si el mes en curso todavia no se ha
    armado (deck plantilla sin numeros), cae automaticamente al mes anterior
    sin intervencion manual."""
    try:
        from pptx import Presentation  # noqa: F401
    except ImportError:
        print('python-pptx no disponible -- P&L Consolidado no se leera de la PPT (fallback).')
        return None
    root = find_monthly_root()
    if root is None:
        print('No se encontro la carpeta "2. Monthly" -- P&L Consolidado no se leera de la PPT (fallback).')
        return None
    for year in _list_year_dirs(root):
        year_dir = os.path.join(root, year)
        for month_name in _list_month_dirs(year_dir):
            month_dir = os.path.join(year_dir, month_name)
            ppt_path = _find_review_pptx(month_dir)
            if not ppt_path:
                continue
            try:
                result = extract_ppt_pl_slide(ppt_path)
            except Exception as e:
                print('  no se pudo leer', ppt_path, '--', e)
                continue
            if result is not None:
                print('  P&L Consolidado leido de la PPT:', ppt_path)
                return result
    print('No se encontro ninguna presentacion mensual con la tabla de P&L ya llena -- fallback a Excel/suma.')
    return None
...
def _build_companies():
    ...
    # Actuals + Budget: prefer the exact numbers from the board deck itself
    # (most recent month's PPT under "2. Monthly", table read live off the
    # slide -- see find_latest_ppt_pl_data above); fall back to the official
    # "Board Outputs" workbook if the PPT isn't reachable/filled in yet, and
    # finally to summing the 4 dashboards' own lines (no eliminations, budget
    # summed from each company) if neither source works. Either way the
    # result is {field: {yoy, prev, cur, ytd, bud_cur, bud_ytd}} -- a scalar
    # snapshot, not a monthly series -- since that's the shape the
    # reconciliation table actually needs.
    ppt_data = find_latest_ppt_pl_data()
    board_actuals = None
    if ppt_data is None:
    ...
    pl_accounting = []
    pl_notes_title_bold = DEFAULT_PL_NOTES_TITLE_BOLD
    pl_notes_title_rest = DEFAULT_PL_NOTES_TITLE_REST
    pl_notes = DEFAULT_PL_NOTES

    if ppt_data is not None:
        consolidated_pl_source = 'ppt'
        consolidated_pl = ppt_data['pl']
        pl_accounting = ppt_data['accounting']
        pl_notes_title_bold = ppt_data['notes_title_bold']
        pl_notes_title_rest = ppt_data['notes_title_rest']
        pl_notes = ppt_data['notes']
    elif board_actuals is not None:
        consolidated_pl_source = 'board_xlsx'
        ...
        consolidated_pl_source = 'summed_fallback'
    ...
    return {
        ...
        'pl_accounting': pl_accounting,
        'pl_notes_title_bold': pl_notes_title_bold,
        'pl_notes_title_rest': pl_notes_title_rest,
        'pl_notes': pl_notes,
    }
