...
DASHBOARD_START_MONTH = '2026-01'  # el consolidado solo muestra datos a partir de este mes


def _trim_to_start_month(data, start_month):
    """Recorta TODO el dataset (meses, KPIs por empresa, P&L consolidado) para
    que arranque en `start_month` -- asi todos los graficos, sparklines y la
    tabla de P&L parten del mismo punto, sin historia anterior."""
    months = data['months']
    ...
    if start_idx == 0:
        return data
    data['months'] = months[start_idx:]
    data['last_actual_idx'] = data['last_actual_idx'] - start_idx
    for comp in data['companies'].values():
        for ki in comp['kpis']:
            ki['data'] = ki['data'][start_idx:]
            ki['spark'] = ki['spark'][start_idx:]
            if ki.get('budget') is not None:
                ki['budget'] = ki['budget'][start_idx:]
    for block in data['consolidated_pl'].values():
        block['actual'] = block['actual'][start_idx:]
        block['budget'] = block['budget'][start_idx:]
    return data
...
def build_consolidated_data():
    ...
    data = _trim_to_start_month(data, DASHBOARD_START_MONTH)
